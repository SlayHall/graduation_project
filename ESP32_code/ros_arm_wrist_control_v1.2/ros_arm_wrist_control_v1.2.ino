...
void wrist_controll(){
  ...
  // linear move
  myPID[2].Compute();
  in_pin1 = (output[2] > 0) ? 4096 : 0;
  in_pin2 = (output[2] > 0) ? 0 : 4096;
  // first mottor
  pwmVal = constrain(abs(output[2]), PWM_MIN, PWM_MAX);
  ...
  // rotational move
  myPID[1].Compute();
  in_pin1 = (output[1] > 0) ? 4096 : 0;
  in_pin2 = (output[1] > 0) ? 0 : 4096;
  // first mottor
  pwmVal = constrain(abs(output[1]), PWM_MIN, PWM_MAX);
  pwm.setPWM(motorChannels[1][1], in1, 4096 - in1);  // IN1
  pwm.setPWM(motorChannels[1][2], in2, 4096 - in2);  // IN2
  pwm.setPWM(motorChannels[1][0], 0, pwmVal);        // PWM
...
void controlMotor(int joint) {
    ...
    if (abs(input[joint] - setpoint[joint]) > DEADBAND) {
        pwmVal = constrain(abs(output[joint]), PWM_MIN, PWM_MAX);
        ...
        if(joint == 5) {
            ...
            in_pin1 = (output[joint] > 0) ? 4096 : 0;
            in_pin2 = (output[joint] > 0) ? 0 : 4096;
